...
export class SmallTimerWidget {
    ...
    start() {
        ...
        this.interval = setInterval(() => {
            ...
            const offset = this.circumference * (remaining / this.duration);
            this.circle.style.strokeDashoffset = offset;
            document.querySelector('.small-time-text').textContent = `${(this.duration - remaining) / 1000}s / ${this.duration / 1000}s`;
            if (remaining <= 0) {
                clearInterval(this.interval);
                this.circle.style.stroke = '#ff9800'; // Change color to orange
    ...
    reset() {
        clearInterval(this.interval);
        this.circle.style.strokeDashoffset = this.circumference;
        document.querySelector('.small-time-text').textContent = `${this.duration / 1000}s / ${this.duration / 1000}s`;
    }
}
